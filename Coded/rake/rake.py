...
def generate_candidate_keywords(sentence_list, stopword_pattern, min_char_length=1, max_words_length=2):
    phrase_list = []
    for s in sentence_list:
        ...
        for phrase in phrases:
            phrase = phrase.strip()
            t=ngrams(phrase.split(),max_words_length)
            for p in t:
                phr=' '.join(p)
                if phr != "" and is_acceptable(phr, min_char_length, max_words_length):
                    #print(phr)
...
def is_acceptable(phrase, min_char_length, max_words_length):
    ...
    if digits >= len(phrase):
        return 0
    return 1


def calculate_word_scores(phraseList,text):
    word_frequency = {}
    word_degree = {}
    for phrase in phraseList:
        ...
        for word in word_list:
            freq=findFrequency(word,text)
            #print(word+"=>"+str(freq))
            word_frequency.setdefault(word, 0)
            word_frequency[word] += 1
            #print (word+" : "+str(word_frequency[word]))
            #word_frequency[word] += freq+1
            word_degree.setdefault(word, 0)
            word_degree[word] += word_list_degree  #orig.
            #print(word + " : " + str(word_degree[word]))
            #word_degree[word] += 1/(word_list_length*1.0) #exp.
    for item in word_frequency:
        word_degree[item] = word_degree[item] + word_frequency[item]
        #print(item + " : " + str(word_degree[item]))

    # Calculate Word scores = deg(w)/frew(w)
    word_score = {}
    for item in word_frequency:
        word_score.setdefault(item, 0)
        if(word_frequency[item]!=0):
...
class Rake(object):
    ...
    def run(self, text):
        ...
        word_scores = calculate_word_scores(phrase_list,text)
        #print(word_scores)
